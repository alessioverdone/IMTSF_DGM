...
class Hi_Patch(nn.Module):
    ...
    def IMTS_Model(self, x, mask_X, x_time):
        ...
        """
        B, N, M, L, D = x.shape  # (8,41,1,43,64)
        ...
        for n_layer in range(1, self.patch_layer):
            ...
            # patch_interval[missing_indices[0], :, missing_indices[1]] = torch.zeros(len(missing_indices[0]), N * T).to(x.device)
            # NEW
            mask_missing = torch.ones_like(patch_interval)
            mask_missing[missing_indices[0], missing_indices[1]] = 0
            mask_missing[missing_indices[0], :, missing_indices[1]] = 0
            patch_interval = patch_interval * mask_missing
    ...
    def forecasting(self, time_steps_to_predict, X, truth_time_steps, mask=None):
        ...
        var_emb = self.nodevec.weight.view(1, N, 1, 1, self.hid_dim).repeat(B, 1, M, L_in, 1)  # variable encoder (8,41,1,57,64 )
        X = self.relu(X + var_emb + te_his)  # node-graph embedding (8,41,1,57,64 )
