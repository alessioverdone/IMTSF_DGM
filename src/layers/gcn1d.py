...
class GCN1DConv(nn.Module):
    def __init__(self, in_dim, out_dim, in_ch, out_ch, hid_dim=128, hid_ch=16, k=3, activation=True, normalization="sym", bias=None):
        ...
        self.block_2 = ChebConv(input_features_cnn=hid_dim,  # 14  #22
                              output_features_cnn=out_dim*2,  # 12  #20
                              kernel_size=k,  # 3  #7
                              params=None,
                              in_channels=hid_ch,
                              out_channels=out_ch,
                              ...
                              normalization=normalization,
                              bias=bias)
        self.bias_2 = Parameter(torch.Tensor(1, out_ch * out_dim*2))
        zeros(self.bias_2)
        self.activation = activation
        if self.activation:
            self.relu = nn.ReLU()
        self.relu = nn.ReLU()
        self.lin_layer = torch.nn.Linear(out_dim*2, out_dim)
