import torch
from src.dataset.utils import get_next_batch
...
def compute_error(truth, pred_y, mask, func, reduce, norm_dict=None):
	...
	# NEW
	truth_repeated = truth.detach().repeat(pred_y.size(0), 1, 1, 1)
	mask = mask.detach().repeat(pred_y.size(0), 1, 1, 1)
