...
from src.dataset.utils import setup_seed, makedirs, get_logger, get_next_batch
from src.dataset.parse_datasets import parse_datasets
from src.dataset.evaluation import *
from src.layers.hipatch import *
import warnings
...
if __name__ == '__main__':
    ...
    for itr in range(args.epoch):
        ...
        for _ in range(num_batches):
            ...
            batch_dict = {k: v.to(args.device) if isinstance(v, torch.Tensor) else v for k, v in batch_dict.items()}
            train_res = compute_all_losses(model, batch_dict)
            train_res["loss"].backward(retain_graph=False)
            optimizer.step()
            # torch.cuda.empty_cache()
            # del train_res, batch_dict
        ...
        model.eval()
        with torch.no_grad():
            val_res = evaluation(model, data_obj["val_dataloader"], data_obj["n_val_batches"], args.device)

            ### Testing ###
            if (val_res["mse"] < best_val_mse):
                best_val_mse = val_res["mse"]
                best_iter = itr
                test_res = evaluation(model, data_obj["test_dataloader"], data_obj["n_test_batches"], args.device)
